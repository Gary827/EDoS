...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			...
			if (cl.getWebServersTimingInfo().getDeparturingTime() <= currentTime) {// finished: remove from the list
				toRemove.add(rcl);
				cloudletFinish(rcl);
				continue;
			}
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			double estimatedFinishTime = rcl.getCloudlet().getWebServersTimingInfo().getDeparturingTime();
	...
	@Override
	public void finishedCloudlet(Cloudlet cl) {
		for (ResCloudlet rcl : getCloudletExecList()) {
			Cloudlet cl2 = rcl.getCloudlet();
			if (cl2 == cl) {// finished: remove from the list
				cloudletFinish(rcl);
				break;
			}
		}
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			double estimatedFinishTime = currentTime
					+ (rcl.getActualRemainingCloudletLength() / (getCapacity(mipsShare) * rcl.getNumberOfPes())) + 0.00001;
	...
	//Ahmad
	@Override
	public long getTotalRemainingCloudletLength(double currentTime) {
		long totalRemaining = 0;

		for (ResCloudlet rcl : getCloudletExecList()) {
			totalRemaining += rcl.getRemainingCloudletLengthInstruction();
		}
